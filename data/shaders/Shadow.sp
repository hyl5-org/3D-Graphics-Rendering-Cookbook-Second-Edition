...
float PCF(vec3 uvw, uint textureid, uint samplerid) {
  return textureBindless2DShadow(textureid, samplerid, uvw);
}
...
float shadow(vec4 s, uint textureid, uint samplerid) {
  s = s / s.w;
  if (s.z > -1.0 && s.z < 1.0) {
    float shadowSample = PCF(vec3(s.x, 1.0 - s.y, s.z), textureid, samplerid);
