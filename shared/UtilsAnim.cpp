...
AnimationChannel initChannel(const aiNodeAnim *anim)
{
    ...
    {
        channel.pos[i] = {.pos = aiVector3DToVec3(anim->mPositionKeys[i].mValue),
    ...
    {
        channel.rot[i] = {.rot = aiQuaternionToQuat(anim->mRotationKeys[i].mValue),
    ...
    {
        channel.scale[i] = {.scale = aiVector3DToVec3(anim->mScalingKeys[i].mValue),
...
{
    return std::max(0, (int)std::distance(t.begin(), std::lower_bound(t.begin(), t.end(), time,
                                                                      [&](const T &lhs, float rhs)
                                                                      {
                                                                          return lhs.time < rhs;
                                                                      })) -
                           1);
...
mat4 animationTransformBlending(const AnimationChannel &channel1, float time1, const AnimationChannel &channel2,
    ...
    for (uint32_t i = 0; i < scene->mNumAnimations; ++i)
    {
        ...
        for (uint32_t c = 0; c < numMorphTargetChannels; c++)
        {
            ...
            for (uint32_t k = 0; k < channel->mNumKeys; ++k)
            {
                ...
                for (uint32_t v = 0; v < std::min((uint32_t)MAX_MORPH_WEIGHTS, channel->mKeys[k].mNumValuesAndWeights);
                     ++v)
                {
...
void updateAnimation(GLTFContext &glTF, AnimationState &anim, float dt)
{
    ...
    // Apply animations
    std::function<void(GLTFNodeRef gltfNode, const mat4 &parentTransform)> traverseTree =
        [&](GLTFNodeRef gltfNode, const mat4 &parentTransform)
    {
        ...
        if (boneId != ~0u)
        {
            ...
            glTF.matrices[glTF.nodesStorage[gltfNode].modelMtxId] =
                parentTransform * (hasActiveChannel ? animationTransform(channel->second, anim.currentTime)
    ...
    if (anim1.active && anim2.active)
    {
            ...
            [&](GLTFNodeRef gltfNode, const mat4 &parentTransform)
        {
            ...
            if (boneId != ~0u)
            {
                ...
                {
                    glTF.matrices[glTF.nodesStorage[gltfNode].modelMtxId] =
                        parentTransform * animationTransformBlending(channel1->second, anim1.currentTime,
                                                                     channel2->second, anim2.currentTime, weight);
                }
                else if (channel1 != activeAnim1.channels.end())
                {
                    glTF.matrices[glTF.nodesStorage[gltfNode].modelMtxId] =
                        parentTransform * animationTransform(channel1->second, anim1.currentTime);
                }
                else if (channel2 != activeAnim2.channels.end())
                {
                    glTF.matrices[glTF.nodesStorage[gltfNode].modelMtxId] =
                        parentTransform * animationTransform(channel2->second, anim2.currentTime);
                }
                else
                {
                    glTF.matrices[glTF.nodesStorage[gltfNode].modelMtxId] =
