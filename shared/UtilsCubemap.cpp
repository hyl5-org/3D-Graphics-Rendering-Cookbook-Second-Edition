...
void convolveLambertian(const vec3 *data, int srcW, int srcH, int dstW, int dstH, vec3 *output,
                        int numMonteCarloSamples)
{
    ...
    stbir_resize(reinterpret_cast<const float *>(data), srcW, srcH, 0, reinterpret_cast<float *>(tmp.data()), dstW,
                 dstH, 0, STBIR_RGB, STBIR_TYPE_FLOAT, STBIR_EDGE_WRAP, STBIR_FILTER_CUBICBSPLINE);
...
Bitmap convertEquirectangularMapToVerticalCross(const Bitmap &b)
{
    ...
    const ivec2 kFaceOffsets[] = {ivec2(faceSize, faceSize * 3), ivec2(0, faceSize), ivec2(faceSize, faceSize),
    ...
    for (int face = 0; face != 6; face++)
    {
        for (int i = 0; i != faceSize; i++)
        {
            for (int j = 0; j != faceSize; j++)
            {
                ...
                const float Uf = float(2.0f * faceSize * (theta + kPi) / kPi);
                const float Vf = float(2.0f * faceSize * (kPi / 2.0f - phi) / kPi);
