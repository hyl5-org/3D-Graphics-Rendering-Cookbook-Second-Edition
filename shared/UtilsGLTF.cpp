...
bool assignUVandSampler(const GLTFGlobalSamplers &samplers, const aiMaterial *mtlDescriptor, aiTextureType textureType,
                        uint32_t &uvIndex, uint32_t &textureSampler, int index)
{
    aiString path;
...
namespace
{
void loadMaterialTexture(const aiMaterial *mtlDescriptor, aiTextureType textureType, const char *assetFolder,
                         lvk::Holder<lvk::TextureHandle> &textureHandle, const std::unique_ptr<lvk::IContext> &ctx,
...
GLTFMaterialDataGPU setupglTFMaterialData(const std::unique_ptr<lvk::IContext> &ctx, const GLTFGlobalSamplers &samplers,
                                          ...
                                          GLTFDataHolder &glTFDataholder, bool &useVolumetric, bool &usedoublesided)
{
    ...
    loadMaterialTexture(mtlDescriptor, aiTextureType_CLEARCOAT, assetFolder, mat.clearCoatRoughnessTexture, ctx, false,
                        1);
    ...
    GLTFMaterialDataGPU res = {
        ...
        .surfacePropertiesTexture =
            mat.surfacePropertiesTexture.valid() ? mat.surfacePropertiesTexture.index() : mat.white.index(),
        ...
        .sheenRoughnessTexture =
            mat.sheenRoughnessTexture.valid() ? mat.sheenRoughnessTexture.index() : mat.white.index(),
        .clearCoatTexture = mat.clearCoatTexture.valid() ? mat.clearCoatTexture.index() : mat.white.index(),
        .clearCoatRoughnessTexture =
            mat.clearCoatRoughnessTexture.valid() ? mat.clearCoatRoughnessTexture.index() : mat.white.index(),
        .clearCoatNormalTexture =
            mat.clearCoatNormalTexture.valid() ? mat.clearCoatNormalTexture.index() : mat.white.index(),
    ...
    assignUVandSampler(samplers, mtlDescriptor, aiTextureType_DIFFUSE, res.baseColorTextureUV,
    ...
    assignUVandSampler(samplers, mtlDescriptor, aiTextureType_EMISSIVE, res.emissiveTextureUV,
    ...
    if (materialTypeFlags & MaterialType_MetallicRoughness)
    {
        ...
        assignUVandSampler(samplers, mtlDescriptor, aiTextureType_METALNESS, res.surfacePropertiesTextureUV,
    ...
    else if (materialTypeFlags & MaterialType_SpecularGlossiness)
    {
        ...
        assignUVandSampler(samplers, mtlDescriptor, aiTextureType_SPECULAR, res.surfacePropertiesTextureUV,
                           res.surfacePropertiesTextureSampler);
    ...
    assignUVandSampler(samplers, mtlDescriptor, aiTextureType_LIGHTMAP, res.occlusionTextureUV,
                       res.occlusionTextureSampler);
    ...
    // Sheen
    {
        ...
        if (assignUVandSampler(samplers, mtlDescriptor, aiTextureType_SHEEN, res.sheenColorTextureUV,
                               res.sheenColorTextureSampler, 0))
        {
            useSheen = true;
        }
        if (assignUVandSampler(samplers, mtlDescriptor, aiTextureType_SHEEN, res.sheenRoughnessTextureUV,
    ...
    // Clear coat
    {
        bool useClearCoat = !mat.clearCoatTexture.empty() || !mat.clearCoatRoughnessTexture.empty() ||
        ...
        if (assignUVandSampler(samplers, mtlDescriptor, aiTextureType_CLEARCOAT, res.clearCoatTextureUV,
                               res.clearCoatNormalTextureSampler, 0))
        {
            useClearCoat = true;
        }

        if (assignUVandSampler(samplers, mtlDescriptor, aiTextureType_CLEARCOAT, res.clearCoatRoughnessTextureUV,
                               res.clearCoatRoughnessTextureSampler, 1))
        {
            useClearCoat = true;
        }

        if (assignUVandSampler(samplers, mtlDescriptor, aiTextureType_CLEARCOAT, res.clearCoatNormalTextureUV,
    ...
    // Specular
    {
        ...
        assignUVandSampler(samplers, mtlDescriptor, aiTextureType_SPECULAR, res.specularTextureUV,
        ...
        {
            res.specularFactors =
                vec4(specularColorFactor.r, specularColorFactor.g, specularColorFactor.b, res.specularFactors.w);
            useSpecular = true;
        }

        assignUVandSampler(samplers, mtlDescriptor, aiTextureType_SPECULAR, res.specularColorTextureUV,
    ...
    // Transmission
    {
        ...
        assignUVandSampler(samplers, mtlDescriptor, aiTextureType_TRANSMISSION, res.transmissionTextureUV,
...

    {
        ...
        assignUVandSampler(samplers, mtlDescriptor, aiTextureType_TRANSMISSION, res.thicknessTextureUV,
...
void loadGLTF(GLTFContext &gltf, const char *glTFName, const char *glTFDataPath)
{
    ...
    for (uint32_t meshId = 0; meshId != scene->mNumMeshes; meshId++)
    {
        ...
        for (uint32_t a = 0; a < m->mNumAnimMeshes; a++)
        {
            ...
            for (uint32_t i = 0; i < mesh->mNumVertices; i++)
            {
                ...
                const aiColor4D c = mesh->mColors[0] ? mesh->mColors[0][i] : aiColor4D(1.0f, 1.0f, 1.0f, 1.0f);
                const aiVector3D uv0 =
                    mesh->mTextureCoords[0] ? mesh->mTextureCoords[0][i] : aiVector3D(0.0f, 0.0f, 0.0f);
                const aiVector3D uv1 =
                    mesh->mTextureCoords[1] ? mesh->mTextureCoords[1][i] : aiVector3D(0.0f, 0.0f, 0.0f);
                morphData.push_back({
    ...
    for (unsigned int mtl = 0; mtl < scene->mNumMaterials; ++mtl)
    {
        ...
        gltf.matPerFrame.materials[mtl] =
            setupglTFMaterialData(ctx, gltf.samplers, mtlDescriptor, glTFDataPath, gltf.glTFDataholder,
                                  gltf.isVolumetricMaterial, gltf.doublesided);
    ...
    std::function<void(const aiNode *rootNode, GLTFNodeRef gltfNode)> traverseTree =
        [&](const aiNode *rootNode, GLTFNodeRef gltfNode)
    {
        for (unsigned int m = 0; m < rootNode->mNumMeshes; ++m)
        {
            ...
            gltf.meshesStorage.push_back({
                ...
                .sortingType =
                    gltf.matPerFrame.materials[mesh->mMaterialIndex].alphaMode == GLTFMaterialDataGPU::AlphaMode_Blend
                        ? SortingType_Transparent
                    : gltf.matPerFrame.materials[mesh->mMaterialIndex].materialTypeFlags & MaterialType_Transmission
                        ? SortingType_Transmission
                        : SortingType_Opaque,
            });
        ...
        for (GLTFNodeRef i = 0; i < rootNode->mNumChildren; i++)
        {
            ...
            const GLTFNode childNode{
                .name = childName,
                .modelMtxId = getNextMtxId(gltf, childName, nonBoneMtxId,
                                           gltf.matrices[gltf.nodesStorage[gltfNode].modelMtxId] *
                                               aiMatrix4x4ToMat4(node->mTransformation)),
    ...
    const lvk::VertexInput vdesc = {
        .attributes =
            {
                {.location = 0, .format = lvk::VertexFormat_Float3, .offset = 0},
...
void sortTransparentNodes(GLTFContext &gltf, const vec3 &cameraPos)
{
    ...
    std::sort(gltf.transparentNodes.begin(), gltf.transparentNodes.end(),
...
void renderGLTF(GLTFContext &gltf, const mat4 &model, const mat4 &view, const mat4 &proj, bool rebuildRenderList)
{
    ...
    const bool isSizeChanged =
        ctx->getDimensions(ctx->getCurrentSwapchainTexture()) != ctx->getDimensions(gltf.offscreenTex[0]);
    ...
    if (gltf.animated)
    {
        ...
        if (gltf.morphing)
        {
            buf.cmdUpdateBuffer(gltf.morphStatesBuffer, 0, gltf.morphStates.size() * sizeof(MorphState),
...

    {
        ...
        const lvk::Framebuffer framebuffer = {
            .color = {{.texture = screenCopy ? gltf.offscreenTex[gltf.currentOffscreenTex]
    ...
    if (!gltf.transmissionNodes.empty() || !gltf.transparentNodes.empty())
    {
        ...
        if (screenCopy)
        {
            ...
            buf.cmdCopyImage(gltf.offscreenTex[gltf.currentOffscreenTex], ctx->getCurrentSwapchainTexture(),
        ...
        buf.cmdBeginRendering(renderPass, framebuffer,
                              {.textures = {lvk::TextureHandle(gltf.offscreenTex[gltf.currentOffscreenTex])}});
        buf.cmdBindVertexBuffer(0, gltf.vertexBuffer, 0);
        buf.cmdBindIndexBuffer(gltf.indexBuffer, lvk::IndexFormat_UI32);
